...
func (idx *HNSWIndex) Insert(ctx context.Context, topicID uint32, localID uint64, fullVec []float32) error {
	...
	for l := idx.getMaxLevel(topicID); l > level; l-- {
...
func (idx *HNSWIndex) getEntryPoint(topicID uint32) uint64 {
	idx.entryPointMu.RLock()
	defer idx.entryPointMu.RUnlock()
	return idx.entryPoints[topicID]
}

func (idx *HNSWIndex) getMaxLevel(topicID uint32) int {
	entryPoint := idx.getEntryPoint(topicID)
	if entryPoint == 0 {
		return 0
	}
	maxLevel, err := idx.readLevel(entryPoint)
	if err != nil {
		return 0
	}
	return maxLevel
...
func (idx *HNSWIndex) SearchInTopic(ctx context.Context, topicID uint32, queryVec []float32, k int) iter.Seq2[SearchResult, error] {
	return func(yield func(SearchResult, error) bool) {
		...
		maxLevel := idx.getMaxLevel(topicID)
		currEntry := entryPoint
